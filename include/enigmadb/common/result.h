...
public:
  ...
  bool has_value() const { return std::holds_alternative<T>(this->data_); };

  T &value() {
    if (auto *val = std::get_if<T>(&this->data_)) {
      return *val;
    }
  ...
  E &err() { return std::get<E>(this->data_); }

  const E &err() const { return std::get<E>(this->data_); }
...
public:
  ...
  bool has_value() const {
    return std::holds_alternative<std::monostate>(this->data_);
  };
