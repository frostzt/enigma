...
ExpectResult<size_t, Error>
PosixIOEngine::append(const FileHandle &fh, const char *buffer, size_t length) {
  errno = 0;
  if (fh.fd() == -1) {
...
ExpectResult<size_t, Error> PosixIOEngine::read(const FileHandle &fh,
  ...
  while (bytes_read < count) {
    ssize_t bytes = ::pread(fh.fd(), buffer + bytes_read, count - bytes_read,
                            offset + bytes_read);
    if (bytes == -1) {
      if (errno == EINTR) {
        continue;
