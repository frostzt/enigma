...
uint8_t decode_uint8(const uint8_t* buffer, size_t offset) { return static_cast<uint8_t>(buffer[offset]); }
...
uint32_t decode_uint32(const uint8_t* buffer, size_t offset) {
    uint32_t total = 0;
    for (size_t i = 0; i < 4; i++) {
        total |= static_cast<uint32_t>(static_cast<uint8_t>(buffer[offset + i])) << ((3 - i) * 8);
    }
    return total;
}
...
uint64_t decode_uint64(const uint8_t* buffer, size_t offset) {
    uint64_t total = 0;
    for (size_t i = 0; i < 8; i++) {
        total |= static_cast<uint64_t>(static_cast<uint8_t>(buffer[offset + i])) << ((7 - i) * 8);
    }
    return total;
}

size_t encode_bytes(const void* data, size_t length, uint8_t* buffer, size_t offset) {
    if (length > 0) {
