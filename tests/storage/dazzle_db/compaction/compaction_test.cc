...
TEST(compaction, size_tiered_compaction_auto) {
    ...
    auto storage_engine_result = dazzle::Dazzle::open(
        engine, "./compaction_tests", 100000,
...
TEST(compaction, full_compaction_manual) {
    ...
    ASSERT_TRUE(
        storage_engine
            ->set_compaction_policy(
                std::make_unique<dazzle::SizeTieredCompactionPolicy>(80, 160))
            .has_value());
    ...
    auto count_initial = std::count_if(
        fs::directory_iterator(testdir.path + "/sst"), fs::directory_iterator{},
        [](const auto& entry) { return fs::is_regular_file(entry); });
    ...
    auto sstr = dazzle::SSTableReader::create(
        engine, dazzle::sst_path(testdir.path, c_res.value().value().value));
